...
#include "nonrealtivistic_wavef.h"
#include "vector.h"
#include "nucleus.h"
...
const std::string DATE = "2017-xx-xx";
...
const int WAVEF_DVCS = 3; const int WAVEF_NR =4;
enum MODE
{
...
{    
        ...
        for (int i=1; i<argc; i++)
        {
            ...
            else if (string(argv[i])=="-wavef")
            {
                if (string(argv[i+1])=="gaus-lc")
					...
					wavef = WAVEF_DVCS;
				else if (string(argv[i+1])=="nonrel")
					wavef = WAVEF_NR;
                else
                {
    ...
    switch (wavef)
    {
		...
		case WAVEF_NR:
			...
			if (waveffile != "")
					fname=waveffile;
			JPsi = new NRWaveFunction(fname); 
			cout << "# Wave function: " << *((NRWaveFunction*)JPsi) << endl;
			break;
        default:
...
cout << "# Total coherent cross section: " << calculator.TotalCoherentCrossSection(Qsqr, bjorkx)*NBGEVSQR << " nb " << endl; 
    ...
    else if (mode == MODE_DSIGMA_DT_A) // d\sigma/dt as a function of A [minA-maxA]
    {
        ...
        for (unsigned int tmpA=minA; tmpA<=maxA; tmpA+=Astep)
        {
            ...
            //REAL result =  calculator.CoherentCrossSection_dt(0,Qsqr, bjorkx); 
			REAL result = 	calculator.TotalCoherentCrossSection(Qsqr, bjorkx);
            cout.precision(3);
            cout << fixed << tmpA;
            cout.precision(14);
            cout << " " << result*NBGEVSQR << endl;
